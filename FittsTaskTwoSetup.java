...
class FittsTaskTwoSetup extends JDialog implements ActionListener, FocusListener, ItemListener
{
	...
	FittsTaskTwoSetup(Frame owner, FittsTaskTwoConfiguration cArg)
	{
		...
		targets = new JComboBox<String>(ODD_NUMBERS);
		targets.setFont(F14);
		targets.addActionListener(this);

		aField = new JTextField(30); // determines width of param panel
		aField.setFont(F14);
		...
		labelPanel.add(randomizeCheckBox);
		labelPanel.add(beepOnErrorCheckBox);
		labelPanel.add(buttonDownCheckBox);
		labelPanel.add(mouseOverCheckBox);

		JPanel paramPanel = new JPanel();
		paramPanel.setLayout(new GridLayout(0, 1, 0, 10));
		paramPanel.add(participantCode);
		paramPanel.add(conditionCode);
		paramPanel.add(blockCode);
		paramPanel.add(targets);
		paramPanel.add(aField);
		paramPanel.add(wField);
		paramPanel.add(errorField);
		paramPanel.add(hysteresisField);
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		if (source == exitButton)
			...
			c.setNumberOfTargets(Integer.parseInt(ODD_NUMBERS[targets.getSelectedIndex()]));
	...
	private void setDefaults()
	{
		...
		c.setNumberOfTargets(cSave.getNumberOfTargets());
		c.setA(cSave.getA());
		c.setW(cSave.getW());
