 ...
 * tabletop display using Fitts’ reciprocal tapping task</a>, by Sasangohar, MacKenzie, and Scott
 * (<i>HFES 2009</i>).
 * <p>
 *
 ...
 * <p>
 *
 * <li><a href="http://www.yorku.ca/mack/eics2009a.html"> An empirical comparison of “Wiimote” gun
 * attachments for pointing tasks</a>, by McArthur, Castellucci, and MacKenzie (<i>EICS 2009</i>).
 * <p>
 *
 * <li><a href="http://www.yorku.ca/mack/hcii2007.html"> Evaluating eye tracking with ISO 9241 –
 ...
 * of the application. For example, if the software is used to compare a mouse, touchpad, and
 * joystick, the mouse condition could be "C01", the touchpad condition "C02", and the joystick
 ...
 * <td valign="top">
 * Spatial Hysteresis
 * <td>A scaling factor to create a larger virtual target &ndash; a hysteresis zone &ndash; to
 * improve target selection. The mouse pointer is deemed to enter the target when it enters the
 ...
 * selection was precisely at the centre of the target. It means selection was on the line
...
public class FittsTaskTwo
{
	...
	private static FittsTaskTwoConfiguration readConfigurationData(String filename) throws IOException
	{
		...
		if (!cfgFile.exists())
		{
			...
			int resp = JOptionPane
					.showConfirmDialog(null, msg, "Missing Configuration File", JOptionPane.YES_NO_OPTION);
		...
		// CONDITION CODE (used in name of output data files)
		while ((line = br.readLine().trim()).length() == 0 || line.charAt(0) == '#')
			continue;
		String conditionCode = line;
		...
		// NUMBER OF TARGETS (Note: number of trials equals the number of targets)
		while ((line = br.readLine().trim()).length() == 0 || line.charAt(0) == '#')
			continue;
		int numberOfTargets = Integer.parseInt(line);
		...
		while ((line = br.readLine().trim()).length() == 0 || line.charAt(0) == '#')
			continue;
		boolean randomize = line.toLowerCase().equals("yes");

		// BEEP ON ERROR (yes, no)
		while ((line = br.readLine().trim()).length() == 0 || line.charAt(0) == '#')
			continue;
		boolean beepOnError = line.toLowerCase().equals("yes");

		// BUTTON-DOWN HIGHLIGHT (yes, no)
		while ((line = br.readLine().trim()).length() == 0 || line.charAt(0) == '#')
			continue;
		boolean buttonDownHighlight = line.toLowerCase().equals("yes");

		// MOUSE-OVER HIGHLIGHT (yes, no)
		while ((line = br.readLine().trim()).length() == 0 || line.charAt(0) == '#')
			continue;
		boolean mouseOverHighlight = line.toLowerCase().equals("yes");

		// ERROR RATE THRESHOLD (%) to repeat sequence
		while ((line = br.readLine().trim()).length() == 0 || line.charAt(0) == '#')
			continue;
		int errorThreshold = Integer.parseInt(line);

		// SPATIAL HYSTERESIS (a double)
		while ((line = br.readLine().trim()).length() == 0 || line.charAt(0) == '#')
			continue;
		double hysteresis = Double.parseDouble(line);

		// BACKGROUND COLOR (for task panel)
		while ((line = br.readLine().trim()).length() == 0 || line.charAt(0) == '#')
			continue;
		st = new StringTokenizer(line, ", ");
		if (st.countTokens() != 3)
		...
		Color backgroundColor = new Color(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()), Integer
		...
		Color foregroundColor = new Color(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()), Integer
				.parseInt(st.nextToken()));
		...
		Color targetColor = new Color(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()), Integer
				.parseInt(st.nextToken()));
		...
		Color buttonDownColor = new Color(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()), Integer
				.parseInt(st.nextToken()));
		...
		Color mouseOverColor = new Color(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()), Integer
				.parseInt(st.nextToken()));
...
class FittsTaskTwoFrame extends JFrame implements MouseMotionListener, MouseListener, ComponentListener
{
	...
	private int numberOfTrials; // number of trials per target condition
	...
	{
		c = cArg;
		numberOfTrials = c.getNumberOfTargets();
		a = c.getA();
		w = c.getW();
		...
		block = new FittsTaskTwoBlock(numberOfSequences, numberOfTrials, a, w, randomize);

		// open sd1, sd2, and sd3 files
		String fileName = "FittsTaskTwo-" + c.getParticipantCode() + "-" + c.getConditionCode() + "-"
				+ c.getBlockCode();

		// Check to see if output data files exist. If so, issue an overwrite warning
		// before proceeding.
		...
		tp = new TaskPanel(a[idxA], w[idxW], 1, numberOfSequences, c.getForegroundColor(), c.getTargetColor(), c
				.getButtonDownColor(), c.getMouseOverColor());
	...
	{
		xClick = new int[numberOfTrials + 1]; // need one extra for initial click to begin sequence
		clickPoint = new Point[numberOfTrials + 1];
	}
	...
	public void mouseReleased(MouseEvent me)
	{
		...
		if (trial < numberOfTrials) // not end of sequence
		{
		...
		// end of sequence, see if it needs to be repeated
		{
			...
			for (int i = 0; i < block.getSequence(block.getIDX()).getTrials(); ++i)
			{
				...
				s = "FittsTaskTwo" + "," + c.getParticipantCode() + "," + c.getConditionCode() + "," + c.getBlockCode()
						+ "," + block.getSequence(block.getIDX()).getTrialData(i) + "\n";
				try
				{
					bw1.write(s, 0, s.length());
					bw1.flush();
	...
	class TaskPanel extends JPanel
	{
				...
				Color buttonDownColorArg, Color mouseOverColorArg)
		{
			...
			target = new Ellipse2D.Double[numberOfTrials];
			for (int i = 0; i < target.length; ++i)
				target[i] = new Ellipse2D.Double(0, 0, 0, 0);
			centerPoint = new Point[numberOfTrials];
			...
			/*
			 * Build an array of indices of the order of highlighted targets. The algorithm is
			 * slightly different if the number of targets is odd. An advantage of using an odd
			 * number of targets is that the movement amplitude is exactly the same for all trials.
			 * If the number of targets is even, the movement amplitude is different from one trial
			 * to the next. "+1" is used because the first selection is used just to start the
			 * sequence.
			 */
			targetOrder = new int[numberOfTrials + 1];
			targetOrder[0] = numberOfTrials / 4 * 3; // 1st target is at the top (3/4 around circle)

			int inc = numberOfTrials / 2 + numberOfTrials % 2;
			if (numberOfTrials % 2 == 0) // even number of targets
				for (int i = 1; i < numberOfTrials + 1; ++i)
					targetOrder[i] = (targetOrder[i - 1] + (numberOfTrials + 1) / 2 + ((i + 1) % 2)) % numberOfTrials;
			else
				// odd number of targets
				for (int i = 1; i < numberOfTrials + 1; ++i)
					targetOrder[i] = (inc + targetOrder[i - 1]) % numberOfTrials;
		...
		private void configure()
		{
			...
			{
				double x = centreX + (a / 2) * Math.cos(2 * Math.PI * ((double)i / numberOfTrials));
				double y = centreY + (a / 2) * Math.sin(2 * Math.PI * ((double)i / numberOfTrials));
