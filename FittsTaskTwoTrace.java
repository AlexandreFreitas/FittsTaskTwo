 ...
 * Measures for Evaluating Computer Pointing Devices</a> by MacKenzie, Kauppinen, and Silfverberg
...
public class FittsTaskTwoTrace
{
	public static void main(String[] args)
	{
		if (args.length > 0)
		{
			System.out.println("Usage: FittsTaskTwoTrace (see API for details)");
			System.exit(0);
		}
		...
		FittsTaskTwoTraceFrame frame = new FittsTaskTwoTraceFrame();
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.setTitle("FittsTaskTwoTrace");
		frame.pack();
		frame.setVisible(true);
	}
}

class FittsTaskTwoTraceFrame extends JFrame implements ActionListener, KeyListener, ItemListener
{
	...
	class TracePanel extends JPanel
	{
		...
		public void drawTargets(Graphics g)
		{
			...
			for (int i = 0; i < t.length; ++i)
			{
				...
				// in trial mode, draw target to select last (to occlude overlapping targets, if
				// any)
				if (sequenceCounter == t[i].getSequence())
				{
		...
		private void paintInfo(Graphics g)
		{
			...
			String sequenceInfo = String.format("Sequence: %d of %d (A=%d, W=%d)", sequenceCounter,
					numberOfSequences(), t[trialIdx].a, t[trialIdx].w);
			...
			// sequence mode
			{
				...
				{
					/*
					 * Note: trialIdx is the index of the first trial in the sequence while j is the
					 * index of the trial within the sequence
					 */
		...
		private int loadData(String fileArg)
		{
			...
			try
			{
				...
				while ((tLine = br.readLine()) != null)
				{
					...
					/*
					 * New format for sd3 file includes 4 more columns on left (Filename,
					 * Participant, Condition, Block). Pass over these now.
					 */
